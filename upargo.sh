...
echo -e "\033\[1;32m    curl -Ls [https://raw.githubusercontent.com/guangwit9/upargo/main/upargo.sh](https://raw.githubusercontent.com/guangwit9/upargo/main/upargo.sh) | less\033\[0m"
echo -e "\033\[1;31m==============================================================\033\[0m"
echo "按任意键继续..."
read -n1 -s
clear
...
\[ -z "\$GIT\_USER" ] && read -p "请输入 GitLab 用户名: " GIT\_USER
\[ -z "\$GIT\_EMAIL" ] && read -p "请输入 GitLab 邮箱: " GIT\_EMAIL
\[ -z "\$PROJECT" ] && read -p "请输入 GitLab 项目名: " PROJECT

TMP\_DIR="/tmp/idx\_upload"
FILES=(
...
git config --global user.name "\$GIT\_USER"
...
git clone [https://oauth2:\$TOKEN@gitlab.com/\$GIT\_USER/\$PROJECT.git](https://oauth2:$TOKEN@gitlab.com/$GIT_USER/$PROJECT.git)
